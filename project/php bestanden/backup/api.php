...
if (strcasecmp($_GET['m'],'padd') == 0) {
	if(!$conn){
        ...
        $response['data'] = mysqli_connect_error();
		 ...
		 $response['code'] = 0;
        $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
		$idQuery = "select max(pr_id) from producten";
		$idMax = $conn -> query($idQuery);
		$lQuery = "insert into producten values(".$idMax","$_POST['name']","$_POST['price']",0)";
		$result = $conn -> query($lQuery);
		$rows = array();
		if (!$result){
			$response['data'] = "db error";	
