...
var myApp = new Framework7({
    ...
    on: {
        pageInit: function (page) {
            ...
            if (page.route.name === "makeRoom") {
                $$('#btnMakeRoom').on('click', function () {
                    makeRoom();
                });
            }
            if (page.route.name === "joinRoom") {
...
function getList() {
    ...
    myApp.request.postJSON(
        'http://localhost/mi3les5/testdb.php?', 
        data, 
...
function makeRoom(){
    ...
    data.id = 11;
    myApp.request.postJSON(
        'http://127.0.0.1/game/public/api/rooms/add',
        data,
        function(responseData, textStatus, jqXHR) {
            if(responseData.status === "fail") {
                myApp.dialog.alert("Sorry, probeer nog een keer met meer data ...", responseData.error);
            } else {
                myApp.dialog.alert("succes", "Kamer gemaakt");
            }
        },
        function(responseData, textStatus, errorThrown) {
            myApp.dialog.alert('POST failed :' + errorThrown, "Kamer maken is niet gelukt");
        },
        "json"
    );
}
...
function joinRoom(){
    ...
    data.roomId = 5;
    myApp.request.postJSON(
        'http://127.0.0.1/game/public/api/rooms/join',
        data,
        function(responseData, textStatus, jqXHR) {
            if(responseData.status === "fail") {
                myApp.dialog.alert("Sorry, probeer nog een keer met meer data ...", responseData.error);
...
meView.router.navigate('/');
        ...
        function(responseData, textStatus, errorThrown) {
            myApp.dialog.alert('POST failed :' + errorThrown, "couldn't join room");
        },
        "json"
    );
}
